...
func ParseRecursive(data []byte) (string, error) {
	mathIndices := htmlMath.FindAllIndex(data, -1)

	if len(mathIndices) > 0 {
		mathChunk := data[mathIndices[0][0]:mathIndices[0][1]]
		mathChunk = wikiOpen.ReplaceAll(mathChunk, []byte("("))
		mathChunk = wikiClose.ReplaceAll(mathChunk, []byte(")"))
		data = htmlMath.ReplaceAll(data, mathChunk)
	}
...
func ParseWiktionaryTemplate(tmplt string) *WiktionaryTemplate {
	...
	for i := 1; i < len(tokens); i++ {
		...
		if (strings.Contains(token, "\"=\"") && strings.Contains(token, "=")) || strings.Contains(token, "=") {
			...
			if stringInSlice(subTokens[1], langList) {
				continue
			}
			param = subTokens[1]
		} else {
